"use strict";(()=>{(function(){let L=document.currentScript;if(!L){console.error("[StyleFlo Widget] Current script context not found.");return}let m=L.getAttribute("data-bot-id");if(!m){console.error('[StyleFlo Widget] Missing required "data-bot-id" attribute on script tag.');return}let c=new URL(L.src).origin,z=`styleflo_session_${m}`,x=localStorage.getItem(z);x||(x="session_"+crypto.randomUUID(),localStorage.setItem(z,x));let p=document.createElement("div");p.id="styleflo-chat-widget",p.style.position="fixed",p.style.bottom="0",p.style.right="0",p.style.zIndex="999999",document.body.appendChild(p);let i=p.attachShadow({mode:"open"}),I=document.createElement("link");I.rel="stylesheet",I.href="https://cdn.jsdelivr.net/npm/tailwindcss@2.2.19/dist/tailwind.min.css",i.appendChild(I);let B=document.createElement("style");B.textContent=`
  ...
  `,i.appendChild(B);let w=document.createElement("div");w.className="font-sans",i.appendChild(w);let u="#4F46E5",j="AI Assistant",N="AI Assistant",D="AI Support Agent",f="/avatars/avatar1.png",F="Hello! How can I help you today?",E='<span style="opacity: 0.6; font-size: 11px;">\u26A1 Powered by <strong>StyleFlo</strong></span>';async function R(){try{let l=await fetch(`${c}/api/chatbots/${m}`);if(l.ok){let e=await l.json();e.name&&(j=e.name),e.primaryColor&&(u=e.primaryColor),N=e.agentName||j,D=e.agentRole||"AI Support Agent",f=e.agentAvatarUrl||"/avatars/avatar1.png",F=e.welcomeMessage||"Hello! How can I help you today?",E=e.brandingHtml||E}}catch(l){console.warn("[StyleFlo Widget] Failed to fetch chatbot config, using defaults:",l)}W()}function W(){let l=document.createElement("button");l.className="fixed bottom-5 right-5 w-14 h-14 rounded-full shadow-2xl flex items-center justify-center cursor-pointer transition-all duration-300 hover:scale-110 focus:outline-none z-50",l.style.backgroundColor=u,l.innerHTML=`
      <!-- Chat Icon -->
    ...
    `,w.appendChild(l);let e=document.createElement("div");e.className="fixed z-50 flex flex-col bg-white overflow-hidden transition-all duration-300 transform scale-90 opacity-0 pointer-events-none origin-bottom-right bottom-20 right-5 rounded-2xl border border-gray-100 shadow-2xl styleflo-chat-window",e.innerHTML=`
      <!-- Header -->
      <div class="p-4 text-white flex items-center justify-between shadow-md shrink-0 z-10" style="background-color: ${u};">
    ...
    `,w.appendChild(e);let v=i.getElementById("styleflo-messages"),_=i.getElementById("styleflo-chat-form"),M=i.getElementById("styleflo-input"),O=i.getElementById("styleflo-close-btn"),k=i.getElementById("styleflo-icon-chat"),C=i.getElementById("styleflo-icon-close"),T=!1;function U(){T=!T,T?(e.classList.remove("scale-90","opacity-0","pointer-events-none"),e.classList.add("scale-100","opacity-100","pointer-events-auto"),k.classList.remove("scale-100"),k.classList.add("scale-0"),C.classList.remove("scale-0"),C.classList.add("scale-100"),M.focus()):(e.classList.remove("scale-100","opacity-100","pointer-events-auto"),e.classList.add("scale-90","opacity-0","pointer-events-none"),k.classList.remove("scale-0"),k.classList.add("scale-100"),C.classList.remove("scale-100"),C.classList.add("scale-0"))}l.addEventListener("click",U),O.addEventListener("click",U);function y(){v.scrollTop=v.scrollHeight}function $(o,g=""){let d=document.createElement("div");if(d.className=o==="user"?"flex justify-end":"flex items-start gap-2.5 w-full",o==="bot"){let s=document.createElement("img");s.src=`${c}${f}`,s.alt="Agent Avatar",s.className="w-7 h-7 rounded-full object-cover bg-white border border-gray-100 flex-shrink-0",d.appendChild(s)}let t=document.createElement("div");if(t.className=o==="user"?"p-3 text-white rounded-2xl rounded-tr-none styleflo-text-15 styleflo-mw-85 shadow-sm leading-relaxed":"p-3 bg-white border border-gray-100 text-gray-800 rounded-2xl rounded-tl-none styleflo-text-15 shadow-sm leading-relaxed w-full",o==="user")t.style.backgroundColor=u,t.textContent=g,d.appendChild(t);else{let s=document.createElement("div");s.className="flex flex-col min-w-0 styleflo-mw-75",t.textContent=g,s.appendChild(t),d.appendChild(s)}return v.appendChild(d),y(),t}function Y(){let o=document.createElement("div");return o.id="styleflo-typing-indicator",o.className="flex items-start gap-2.5",o.innerHTML=`
      ...
      `,v.appendChild(o),y(),o}_.addEventListener("submit",async o=>{o.preventDefault();let g=M.value.trim();if(!g)return;$("user",g),M.value="";let d=Y();try{let t=await fetch(`${c}/api/chat/stream`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({message:g,chatbotId:m,sessionId:x})});if(d.remove(),!t.ok)throw new Error(`Server returned HTTP ${t.status}`);let s=t.body?.getReader(),q=new TextDecoder;if(!s){$("bot","Sorry, I am unable to process that message right now.");return}let H=$("bot",""),A="";for(;;){let{done:n,value:r}=await s.read();if(n)break;let h=q.decode(r,{stream:!0});A+=h;let b=A.replace(/\*\*(.*?)\*\*/g,"<strong>$1</strong>").replace(/\[([^\]]+)\]\((https?:\/\/[^)]+)\)/g,'<a href="$2" target="_blank" class="font-semibold text-indigo-600 hover:underline" style="color: ${primaryColor}; text-decoration: underline;">$1</a>').replace(/\n/g,"<br/>");H.innerHTML=b,y()}let G=/\[([^\]]+)\]\((https?:\/\/[^)]+)\)/g,P,S=[];for(;(P=G.exec(A))!==null;){let n=P[2];(n.includes("/products/")||n.includes("/product/")||n.includes("/shop/"))&&!S.includes(n)&&S.push(n)}for(let n of S){let r=document.createElement("div");r.className="mt-3 p-3 bg-gray-50 border border-gray-100 rounded-2xl flex items-center gap-3 styleflo-animate-pulse",r.innerHTML=`
          ...
          `,H.parentNode?.insertBefore(r,H.nextSibling),y();try{let h=await fetch(`${c}/api/products/metadata?url=${encodeURIComponent(n)}`);if(h.ok){let b=await h.json();if(b.success&&b.metadata){let a=b.metadata;r.className="mt-3 p-3 bg-gray-50 border border-gray-100 rounded-2xl flex items-center gap-3 transition-all duration-300 hover:shadow-md",r.innerHTML=`
                  ${a.image_url?`
                  ...
                  <a href="${n}" target="_blank" class="px-3.5 py-1.5 rounded-xl text-[10px] font-bold text-white transition-opacity flex-shrink-0" style="background-color: ${u}; font-size: 10px; font-weight: 700; padding: 6px 12px; border-radius: 8px; text-decoration: none; display: inline-block;">
                    Buy Now
                  </a>
                `}else r.remove()}else r.remove()}catch(h){console.warn("[Widget] Failed to fetch product card details:",h),r.remove()}y()}}catch(t){console.error("[StyleFlo Widget] Chat Stream fetch error:",t),d.remove(),$("bot","An error occurred. Please try again or refresh the page.")}})}R()})();})();
