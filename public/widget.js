"use strict";(()=>{(function(){let B=document.currentScript;if(!B){console.error("[StyleFlo Widget] Current script context not found.");return}let w=B.getAttribute("data-bot-id");if(!w){console.error('[StyleFlo Widget] Missing required "data-bot-id" attribute on script tag.');return}let m=new URL(B.src).origin,U=`styleflo_session_${w}`,C=localStorage.getItem(U);C||(C="session_"+crypto.randomUUID(),localStorage.setItem(U,C));let y=document.createElement("div");y.id="styleflo-chat-widget",y.style.position="fixed",y.style.bottom="0",y.style.right="0",y.style.zIndex="999999",document.body.appendChild(y);let a=y.attachShadow({mode:"open"}),N=document.createElement("link");N.rel="stylesheet",N.href="https://cdn.jsdelivr.net/npm/tailwindcss@2.2.19/dist/tailwind.min.css",a.appendChild(N);let O=document.createElement("style");O.textContent=`
  ...
  `,a.appendChild(O);let E=document.createElement("div");E.className="font-sans",a.appendChild(E);let l="#4F46E5",P="AI Assistant",R="AI Assistant",W="AI Support Agent",v="/avatars/avatar1.png",q="Hello! How can I help you today?",j='<span style="opacity: 0.6; font-size: 11px;">\u26A1 Powered by <strong>StyleFlo</strong></span>';async function Y(){try{let c=await fetch(`${m}/api/chatbots/${w}`);if(c.ok){let t=await c.json();t.name&&(P=t.name),t.primaryColor&&(l=t.primaryColor),R=t.agentName||P,W=t.agentRole||"AI Support Agent",v=t.agentAvatarUrl||"/avatars/avatar1.png",q=t.welcomeMessage||"Hello! How can I help you today?",j=t.brandingHtml||j}}catch(c){console.warn("[StyleFlo Widget] Failed to fetch chatbot config, using defaults:",c)}V()}function V(){let c=document.createElement("button");c.className="fixed bottom-5 right-5 w-14 h-14 rounded-full shadow-2xl flex items-center justify-center cursor-pointer transition-all duration-300 hover:scale-110 focus:outline-none z-50",c.style.backgroundColor=l,c.innerHTML=`
      <!-- Chat Icon -->
    ...
    `,E.appendChild(t);let b=a.getElementById("styleflo-messages"),L=a.getElementById("styleflo-chat-form"),k=a.getElementById("styleflo-input"),Q=a.getElementById("styleflo-close-btn"),$=a.getElementById("styleflo-icon-chat"),I=a.getElementById("styleflo-icon-close"),G=a.getElementById("styleflo-onboarding"),X=a.getElementById("styleflo-onboarding-form"),Z=a.getElementById("styleflo-onboarding-name"),D=localStorage.getItem("styleflo-client-name");D||(b.style.display="none",L.style.display="none",G.style.display="flex"),X.addEventListener("submit",r=>{r.preventDefault();let d=Z.value.trim();d&&(localStorage.setItem("styleflo-client-name",d),D=d,G.style.display="none",b.style.display="block",L.style.display="flex",k.focus())});let z=!1;function J(){z=!z,z?(t.classList.remove("scale-90","opacity-0","pointer-events-none"),t.classList.add("scale-100","opacity-100","pointer-events-auto"),$.classList.remove("scale-100"),$.classList.add("scale-0"),I.classList.remove("scale-0"),I.classList.add("scale-100"),k.focus()):(t.classList.remove("scale-100","opacity-100","pointer-events-auto"),t.classList.add("scale-90","opacity-0","pointer-events-none"),$.classList.remove("scale-0"),$.classList.add("scale-100"),I.classList.remove("scale-100"),I.classList.add("scale-0"))}c.addEventListener("click",J),Q.addEventListener("click",J);function x(){b.scrollTop=b.scrollHeight}function T(r,d=""){let u=document.createElement("div");if(u.className=r==="user"?"flex justify-end w-full":"flex items-start gap-2.5 w-full",r==="bot"){let i=document.createElement("img");i.src=`${m}${v}`,i.alt="Agent Avatar",i.className="w-7 h-7 rounded-full object-cover bg-white border border-gray-100 flex-shrink-0",u.appendChild(i)}let n=document.createElement("div");if(n.className=r==="user"?"p-3 text-white rounded-2xl rounded-tr-none styleflo-text-15 styleflo-mw-85 shadow-sm leading-relaxed w-full":"p-3 bg-white border border-gray-100 text-gray-800 rounded-2xl rounded-tl-none styleflo-text-15 shadow-sm leading-relaxed w-full",r==="user")n.style.backgroundColor=l,n.textContent=d,u.appendChild(n);else{let i=document.createElement("div");i.className="flex flex-col min-w-0 styleflo-mw-75",n.textContent=d,i.appendChild(n),u.appendChild(i)}return b.appendChild(u),x(),n}function ee(){let r=document.createElement("div");return r.id="styleflo-typing-indicator",r.className="flex items-start gap-2.5",r.innerHTML=`
      ...
      `,b.appendChild(r),x(),r}L.addEventListener("submit",async r=>{r.preventDefault();let d=k.value.trim();if(!d)return;T("user",d),k.value="";let u=ee();try{let n=await fetch(`${m}/api/chat/stream`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({message:d,chatbotId:w,sessionId:C,clientName:D})});if(u.remove(),!n.ok)throw new Error(`Server returned HTTP ${n.status}`);let i=n.body?.getReader(),te=new TextDecoder;if(!i){T("bot","Sorry, I am unable to process that message right now.");return}let M=T("bot",""),S="";for(;;){let{done:s,value:o}=await i.read();if(s)break;let g=te.decode(o,{stream:!0});S+=g;let f=S.replace(/\[CHECK_AVAILABILITY:.*?\]/g,"").replace(/\[BOOK_MEETING:.*?\]/g,"").replace(/\[LEAD_CAPTURED:.*?\]/g,"").replace(/\[TIME_SLOTS:.*?\]/g,"").replace(/\*\*(.*?)\*\*/g,"<strong>$1</strong>").replace(/\[([^\]]+)\]\((https?:\/\/[^)]+)\)/g,'<a href="$2" target="_blank" class="font-semibold text-indigo-600 hover:underline" style="color: ${primaryColor}; text-decoration: underline;">$1</a>').replace(/\n/g,"<br/>");M.innerHTML=f,x()}let oe=/\[([^\]]+)\]\((https?:\/\/[^)]+)\)/g,K,F=[];for(;(K=oe.exec(S))!==null;){let s=K[2];(s.includes("/products/")||s.includes("/product/")||s.includes("/shop/"))&&!F.includes(s)&&F.push(s)}for(let s of F){let o=document.createElement("div");o.className="my-2 p-3 bg-gray-50 border border-gray-100 rounded-2xl flex items-center gap-3 styleflo-animate-pulse",o.innerHTML=`
          ...
          `;let g=M.querySelector(`a[href="${s}"]`);g?g.parentNode?.insertBefore(o,g.nextSibling):M.appendChild(o),x();try{let f=await fetch(`${m}/api/products/metadata?url=${encodeURIComponent(s)}`);if(f.ok){let p=await f.json();if(p.success&&p.metadata){let e=p.metadata;o.className="my-2 p-3 bg-gray-50 border border-gray-100 rounded-2xl flex items-center gap-3 transition-all duration-300 hover:shadow-md",o.innerHTML=`
                  ${e.image_url?`
                ...
                `}else o.remove()}else o.remove()}catch(f){console.warn("[Widget] Failed to fetch product card details:",f),o.remove()}x()}let se=/\[TIME_SLOTS:\s*({.*?})\]/,_=S.match(se);if(_&&_[1])try{let s=JSON.parse(_[1]),o=document.createElement("div");o.className="mt-3 w-full";let g="";for(let[p,e]of Object.entries(s))if(Array.isArray(e)&&e.length>0){let A=new Date(p),h=isNaN(A.getTime())?p:A.toLocaleDateString("en-GB",{weekday:"short",month:"short",day:"numeric"});g+=`
                  <div class="mb-3">
                ...
                `}o.innerHTML=g,M.appendChild(o),o.querySelectorAll(".styleflo-time-btn").forEach(p=>{let e=p;e.addEventListener("click",A=>{let h=A.currentTarget,H=h.getAttribute("data-date"),le=h.getAttribute("data-time");h.style.backgroundColor=l,h.style.color="white",h.style.borderColor=l,k.value=`I would like to book ${H} at ${le}`,L.dispatchEvent(new Event("submit"))}),e.addEventListener("mouseenter",()=>{e.style.backgroundColor=l,e.style.borderColor=l,e.style.color="white"}),e.addEventListener("mouseleave",()=>{e.style.backgroundColor="white",e.style.borderColor="#e5e7eb",e.style.color="#374151"})}),x()}catch(s){console.error("[StyleFlo Widget] Failed to parse TIME_SLOTS",s)}}catch(n){console.error("[StyleFlo Widget] Chat Stream fetch error:",n),u.remove(),T("bot","An error occurred. Please try again or refresh the page.")}})}Y()})();})();
