"use strict";(()=>{(function(){let v=document.currentScript;if(!v){console.error("[StyleFlo Widget] Current script context not found.");return}let p=v.getAttribute("data-bot-id");if(!p){console.error('[StyleFlo Widget] Missing required "data-bot-id" attribute on script tag.');return}let i=new URL(v.src).origin,L=`styleflo_session_${p}`,m=localStorage.getItem(L);m||(m="session_"+crypto.randomUUID(),localStorage.setItem(L,m));let n=document.createElement("div");n.id="styleflo-chat-widget",n.style.position="fixed",n.style.bottom="0",n.style.right="0",n.style.zIndex="999999",document.body.appendChild(n);let r=n.attachShadow({mode:"open"}),w=document.createElement("link");w.rel="stylesheet",w.href="https://cdn.jsdelivr.net/npm/tailwindcss@2.2.19/dist/tailwind.min.css",r.appendChild(w);let I=document.createElement("style");I.textContent=`
  ...
  `,r.appendChild(I);let h=document.createElement("div");h.className="font-sans",r.appendChild(h);let c="#4F46E5",E="AI Assistant",M="AI Assistant",A="AI Support Agent",d="/avatars/avatar1.png",T="Hello! How can I help you today?";async function $(){try{let s=await fetch(`${i}/api/chatbots/${p}`);if(s.ok){let e=await s.json();e.name&&(E=e.name),e.primaryColor&&(c=e.primaryColor),M=e.agentName||E,A=e.agentRole||"AI Support Agent",d=e.agentAvatarUrl||"/avatars/avatar1.png",T=e.welcomeMessage||"Hello! How can I help you today?"}}catch(s){console.warn("[StyleFlo Widget] Failed to fetch chatbot config, using defaults:",s)}S()}function S(){let s=document.createElement("button");s.className="fixed bottom-5 right-5 w-14 h-14 rounded-full shadow-2xl flex items-center justify-center cursor-pointer transition-all duration-300 hover:scale-110 focus:outline-none z-50",s.style.backgroundColor=c,s.innerHTML=`
      <!-- Chat Icon -->
    ...
    `,h.appendChild(e);let g=r.getElementById("styleflo-messages"),B=r.getElementById("styleflo-chat-form"),x=r.getElementById("styleflo-input"),j=r.getElementById("styleflo-close-btn"),f=r.getElementById("styleflo-icon-chat"),y=r.getElementById("styleflo-icon-close"),k=!1;function H(){k=!k,k?(e.classList.remove("scale-90","opacity-0","pointer-events-none"),e.classList.add("scale-100","opacity-100","pointer-events-auto"),f.classList.remove("scale-100"),f.classList.add("scale-0"),y.classList.remove("scale-0"),y.classList.add("scale-100"),x.focus()):(e.classList.remove("scale-100","opacity-100","pointer-events-auto"),e.classList.add("scale-90","opacity-0","pointer-events-none"),f.classList.remove("scale-0"),f.classList.add("scale-100"),y.classList.remove("scale-100"),y.classList.add("scale-0"))}s.addEventListener("click",H),j.addEventListener("click",H);function C(){g.scrollTop=g.scrollHeight}function b(t,u=""){let a=document.createElement("div");if(a.className=t==="user"?"flex justify-end":"flex items-start gap-2.5",t==="bot"){let l=document.createElement("img");l.src=`${i}${d}`,l.alt="Agent Avatar",l.className="w-7 h-7 rounded-full object-cover bg-white border border-gray-100 flex-shrink-0",a.appendChild(l)}let o=document.createElement("div");return o.className=t==="user"?"p-3 text-white rounded-2xl rounded-tr-none text-sm max-w-[85%] shadow-sm leading-relaxed":"p-3 bg-white border border-gray-100 text-gray-800 rounded-2xl rounded-tl-none text-sm max-w-[75%] shadow-sm leading-relaxed",t==="user"&&(o.style.backgroundColor=c),o.textContent=u,a.appendChild(o),g.appendChild(a),C(),o}function D(){let t=document.createElement("div");return t.id="styleflo-typing-indicator",t.className="flex items-start gap-2.5",t.innerHTML=`
      ...
      `,g.appendChild(t),C(),t}B.addEventListener("submit",async t=>{t.preventDefault();let u=x.value.trim();if(!u)return;b("user",u),x.value="";let a=D();try{let o=await fetch(`${i}/api/chat/stream`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({message:u,chatbotId:p,sessionId:m})});if(a.remove(),!o.ok)throw new Error(`Server returned HTTP ${o.status}`);let l=o.body?.getReader(),N=new TextDecoder;if(!l){b("bot","Sorry, I am unable to process that message right now.");return}let F=b("bot","");for(;;){let{done:U,value:W}=await l.read();if(U)break;let z=N.decode(W,{stream:!0});F.textContent+=z,C()}}catch(o){console.error("[StyleFlo Widget] Chat Stream fetch error:",o),a.remove(),b("bot","An error occurred. Please try again or refresh the page.")}})}$()})();})();
