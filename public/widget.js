"use strict";(()=>{(function(){let B=document.currentScript;if(!B){console.error("[StyleFlo Widget] Current script context not found.");return}let w=B.getAttribute("data-bot-id");if(!w){console.error('[StyleFlo Widget] Missing required "data-bot-id" attribute on script tag.');return}let m=new URL(B.src).origin,U=`styleflo_session_${w}`,C=localStorage.getItem(U);C||(C="session_"+crypto.randomUUID(),localStorage.setItem(U,C));let y=document.createElement("div");y.id="styleflo-chat-widget",y.style.position="fixed",y.style.bottom="0",y.style.right="0",y.style.zIndex="999999",document.body.appendChild(y);let a=y.attachShadow({mode:"open"}),N=document.createElement("link");N.rel="stylesheet",N.href="https://cdn.jsdelivr.net/npm/tailwindcss@2.2.19/dist/tailwind.min.css",a.appendChild(N);let O=document.createElement("style");O.textContent=`
  ...
  `,a.appendChild(O);let E=document.createElement("div");E.className="font-sans",a.appendChild(E);let r="#4F46E5",P="AI Assistant",R="AI Assistant",W="AI Support Agent",v="/avatars/avatar1.png",q="Hello! How can I help you today?",j='<span style="opacity: 0.6; font-size: 11px;">\u26A1 Powered by <strong>StyleFlo</strong></span>';async function K(){try{let c=await fetch(`${m}/api/chatbots/${w}`);if(c.ok){let t=await c.json();t.name&&(P=t.name),t.primaryColor&&(r=t.primaryColor),R=t.agentName||P,W=t.agentRole||"AI Support Agent",v=t.agentAvatarUrl||"/avatars/avatar1.png",q=t.welcomeMessage||"Hello! How can I help you today?",j=t.brandingHtml||j}}catch(c){console.warn("[StyleFlo Widget] Failed to fetch chatbot config, using defaults:",c)}V()}function V(){let c=document.createElement("button");c.className="fixed bottom-5 right-5 w-14 h-14 rounded-full shadow-2xl flex items-center justify-center cursor-pointer transition-all duration-300 hover:scale-110 focus:outline-none z-50",c.style.backgroundColor=r,c.innerHTML=`
      <!-- Chat Icon -->
    ...
    `,E.appendChild(c);let t=document.createElement("div");t.className="fixed z-50 flex flex-col bg-white overflow-hidden transition-all duration-300 transform scale-90 opacity-0 pointer-events-none origin-bottom-right bottom-20 right-5 rounded-2xl border border-gray-100 shadow-2xl styleflo-chat-window",t.innerHTML=`
      <!-- Header -->
      <div class="p-4 text-white flex items-center justify-between shadow-md shrink-0 z-10" style="background-color: ${r};">
      ...
      <div id="styleflo-onboarding" class="flex-1 flex flex-col items-center justify-center p-6 bg-gray-50 text-center" style="display: none;">
        <div class="w-16 h-16 rounded-full flex items-center justify-center mb-4 text-white shadow-lg" style="background-color: ${r};">
        ...
        <form id="styleflo-onboarding-form" class="w-full">
          <input type="text" id="styleflo-onboarding-name" required placeholder="Your full name" class="w-full px-4 py-3 mb-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:border-transparent transition-all" style="--tw-ring-color: ${r};" />
          <button type="submit" class="w-full py-3 rounded-xl text-white font-semibold shadow-md transition-opacity hover:opacity-95" style="background-color: ${r};">Start Chatting</button>
        </form>
      </div>
    ...
    `,E.appendChild(t);let b=a.getElementById("styleflo-messages"),L=a.getElementById("styleflo-chat-form"),k=a.getElementById("styleflo-input"),Q=a.getElementById("styleflo-close-btn"),$=a.getElementById("styleflo-icon-chat"),I=a.getElementById("styleflo-icon-close"),Y=a.getElementById("styleflo-onboarding"),X=a.getElementById("styleflo-onboarding-form"),Z=a.getElementById("styleflo-onboarding-name"),D=localStorage.getItem("styleflo-client-name");D||(b.style.display="none",L.style.display="none",Y.style.display="flex"),X.addEventListener("submit",s=>{s.preventDefault();let d=Z.value.trim();d&&(localStorage.setItem("styleflo-client-name",d),D=d,Y.style.display="none",b.style.display="flex",L.style.display="flex",k.focus())});let z=!1;function G(){z=!z,z?(t.classList.remove("scale-90","opacity-0","pointer-events-none"),t.classList.add("scale-100","opacity-100","pointer-events-auto"),$.classList.remove("scale-100"),$.classList.add("scale-0"),I.classList.remove("scale-0"),I.classList.add("scale-100"),k.focus()):(t.classList.remove("scale-100","opacity-100","pointer-events-auto"),t.classList.add("scale-90","opacity-0","pointer-events-none"),$.classList.remove("scale-0"),$.classList.add("scale-100"),I.classList.remove("scale-100"),I.classList.add("scale-0"))}c.addEventListener("click",G),Q.addEventListener("click",G);function x(){b.scrollTop=b.scrollHeight}function T(s,d=""){let u=document.createElement("div");if(u.className=s==="user"?"flex justify-end w-full":"flex items-start gap-2.5 w-full",s==="bot"){let i=document.createElement("img");i.src=`${m}${v}`,i.alt="Agent Avatar",i.className="w-7 h-7 rounded-full object-cover bg-white border border-gray-100 flex-shrink-0",u.appendChild(i)}let n=document.createElement("div");if(n.className=s==="user"?"p-3 text-white rounded-2xl rounded-tr-none styleflo-text-15 styleflo-mw-85 shadow-sm leading-relaxed w-full":"p-3 bg-white border border-gray-100 text-gray-800 rounded-2xl rounded-tl-none styleflo-text-15 shadow-sm leading-relaxed w-full",s==="user")n.style.backgroundColor=r,n.textContent=d,u.appendChild(n);else{let i=document.createElement("div");i.className="flex flex-col min-w-0 styleflo-mw-75",n.textContent=d,i.appendChild(n),u.appendChild(i)}return b.appendChild(u),x(),n}function ee(){let s=document.createElement("div");return s.id="styleflo-typing-indicator",s.className="flex items-start gap-2.5",s.innerHTML=`
      ...
      `,b.appendChild(s),x(),s}L.addEventListener("submit",async s=>{s.preventDefault();let d=k.value.trim();if(!d)return;T("user",d),k.value="";let u=ee();try{let n=await fetch(`${m}/api/chat/stream`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({message:d,chatbotId:w,sessionId:C,clientName:D})});if(u.remove(),!n.ok)throw new Error(`Server returned HTTP ${n.status}`);let i=n.body?.getReader(),te=new TextDecoder;if(!i){T("bot","Sorry, I am unable to process that message right now.");return}let M=T("bot",""),S="";for(;;){let{done:l,value:o}=await i.read();if(l)break;let g=te.decode(o,{stream:!0});S+=g;let f=S.replace(/\[CHECK_AVAILABILITY:.*?\]/g,"").replace(/\[BOOK_MEETING:.*?\]/g,"").replace(/\[LEAD_CAPTURED:.*?\]/g,"").replace(/\[TIME_SLOTS:.*?\]/g,"").replace(/\*\*(.*?)\*\*/g,"<strong>$1</strong>").replace(/\[([^\]]+)\]\((https?:\/\/[^)]+)\)/g,'<a href="$2" target="_blank" class="font-semibold text-indigo-600 hover:underline" style="color: ${primaryColor}; text-decoration: underline;">$1</a>').replace(/\n/g,"<br/>");M.innerHTML=f,x()}let oe=/\[([^\]]+)\]\((https?:\/\/[^)]+)\)/g,J,F=[];for(;(J=oe.exec(S))!==null;){let l=J[2];(l.includes("/products/")||l.includes("/product/")||l.includes("/shop/"))&&!F.includes(l)&&F.push(l)}for(let l of F){let o=document.createElement("div");o.className="my-2 p-3 bg-gray-50 border border-gray-100 rounded-2xl flex items-center gap-3 styleflo-animate-pulse",o.innerHTML=`
          ...
          `;let g=M.querySelector(`a[href="${l}"]`);g?g.parentNode?.insertBefore(o,g.nextSibling):M.appendChild(o),x();try{let f=await fetch(`${m}/api/products/metadata?url=${encodeURIComponent(l)}`);if(f.ok){let p=await f.json();if(p.success&&p.metadata){let e=p.metadata;o.className="my-2 p-3 bg-gray-50 border border-gray-100 rounded-2xl flex items-center gap-3 transition-all duration-300 hover:shadow-md",o.innerHTML=`
                  ${e.image_url?`
                  ...
                  <a href="${l}" target="_blank" class="px-3.5 py-1.5 rounded-xl text-[10px] font-bold text-white transition-opacity flex-shrink-0" style="background-color: ${r}; font-size: 10px; font-weight: 700; padding: 6px 12px; border-radius: 8px; text-decoration: none; display: inline-block;">
                    Buy Now
                  </a>
                `}else o.remove()}else o.remove()}catch(f){console.warn("[Widget] Failed to fetch product card details:",f),o.remove()}x()}let le=/\[TIME_SLOTS:\s*({.*?})\]/,_=S.match(le);if(_&&_[1])try{let l=JSON.parse(_[1]),o=document.createElement("div");o.className="mt-3 w-full";let g="";for(let[p,e]of Object.entries(l))if(Array.isArray(e)&&e.length>0){let A=new Date(p),h=isNaN(A.getTime())?p:A.toLocaleDateString("en-GB",{weekday:"short",month:"short",day:"numeric"});g+=`
                  <div class="mb-3">
                ...
                `}o.innerHTML=g,M.appendChild(o),o.querySelectorAll(".styleflo-time-btn").forEach(p=>{let e=p;e.addEventListener("click",A=>{let h=A.currentTarget,H=h.getAttribute("data-date"),re=h.getAttribute("data-time");h.style.backgroundColor=r,h.style.color="white",h.style.borderColor=r,k.value=`I would like to book ${H} at ${re}`,L.dispatchEvent(new Event("submit"))}),e.addEventListener("mouseenter",()=>{e.style.backgroundColor=r,e.style.borderColor=r,e.style.color="white"}),e.addEventListener("mouseleave",()=>{e.style.backgroundColor="white",e.style.borderColor="#e5e7eb",e.style.color="#374151"})}),x()}catch(l){console.error("[StyleFlo Widget] Failed to parse TIME_SLOTS",l)}}catch(n){console.error("[StyleFlo Widget] Chat Stream fetch error:",n),u.remove(),T("bot","An error occurred. Please try again or refresh the page.")}})}K()})();})();
